...
class CustomerJdbcRepositoryTest {
	...
	void can_insert_customer_test() {
		...
		final Optional<Customer> retrievedCustomer = customerJdbcRepository.findById(newCustomer.getCustomerId());
	...
	void testFindAllById() {
		final Optional<Customer> customer = customerJdbcRepository.findById(newCustomer.getCustomerId());
	...
	void testFindAllByName() {
		final Optional<Customer> customer = customerJdbcRepository.findByName(newCustomer.getName());
		assertThat(customer.isEmpty(), is(false));

		final Optional<Customer> unknown = customerJdbcRepository.findByName("unknown-user");
	...
	void testFindAllByEmail() {
		final Optional<Customer> customer = customerJdbcRepository.findByEmail(newCustomer.getEmail());
		assertThat(customer.isEmpty(), is(false));

		final Optional<Customer> unknown = customerJdbcRepository.findByEmail("unknown-user@gmail.com");
		assertThat(unknown.isEmpty(), is(true));
	}
	...
	void updateCustomer() {
		...
		final Optional<Customer> updatedCustomer = customerJdbcRepository.findById(newCustomer.getCustomerId());
